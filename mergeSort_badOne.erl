...
mergeSort(List) ->
	Length = length(List),
	splitL(1,trunc(Length/2),Length,List).
	
%% 分开	
splitL(_Left,Mid,Right,List) when Right == 2 ->	%% like List == [4,9]
	{LeftL,RightL} = lists:split(Mid,List),
	io:format("LeftL:~p,RightL:~p~n",[LeftL,RightL]),
	mergeL(1,LeftL,1,RightL,[]);		%% Here we shouldn't call the mergeL() function any more
splitL(Left,Mid,_Right,List) ->				%%[Left...Mid][Mid+1...Right]
	{LeftL,RightL} = lists:split(Mid,List),
	LeftResult = splitL(Left,trunc(length(LeftL)/2),Mid,LeftL),
	RightResult = splitL(1,trunc(length(RightL)/2),length(RightL),RightL),
